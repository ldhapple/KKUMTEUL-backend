package com.kkumteul.domain.history.repository;

import com.kkumteul.domain.history.entity.ChildPersonalityHistory;
import com.kkumteul.domain.history.entity.ChildPersonalityHistoryTopic;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface ChildPersonalityHistoryTopicRepository extends JpaRepository<ChildPersonalityHistoryTopic, Long> {
    List<ChildPersonalityHistoryTopic> findByHistoryId(Long id);
}
